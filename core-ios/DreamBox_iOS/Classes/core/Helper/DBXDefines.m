...
+ (CGFloat)db_getUnit:(NSString *)originUnitStr pathId:(NSString *)pathId{
    NSString *unitStr = originUnitStr;
    if([originUnitStr containsString:@"$"]){
        unitStr =[DBXParser getRealValueByPathId:pathId andKey:unitStr];
    }
    
    if ([DBXValidJudge isValidString:unitStr]) {
