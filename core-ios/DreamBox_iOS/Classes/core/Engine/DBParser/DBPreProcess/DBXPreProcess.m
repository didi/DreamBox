...
@implementation DBXPreProcess

+ (DBXProcesssModel *)preProcess:(NSString *)data {
    
    if (![DBXValidJudge isValidString:data]) {
        [DBXLog log:DBLogLevelDebug tag:@"Parameter valid" msg:@"data is nil"];
        return nil;
    }
    
    NSUInteger len = kDB_MD5_INFO_LEN + kDB_CLI_VER_LEN + kDB_SDK_VER_LEN + kDB_EMPTY_INFO_LEN;
    if (data.length < len) {
        [DBXLog log:DBLogLevelDebug tag:@"Parameter valid" msg:@"data is wrong"];
        return nil;
    }
    
    DBXProcesssModel *process = [[DBXProcesssModel alloc] init];
    ...
    // version校验
    if (![DBXPreProcess isValidVersion:process.sdk_ver_info]) {
        [DBXLog log:DBLogLevelDebug tag:@"version valid" msg:@"version check is fail"];
        return nil;
    }
    ...
    if ([process.md5_info isEqualToString:md5]) {
        ...
        [DBXLog log:DBLogLevelDebug tag:@"md5 valid" msg:@"md5 check is fail"];
        return nil;
    }
...
+ (BOOL)isValidVersion:(NSString *)version {
    ...
    NSString *currentVerStr = [DBXDefines db_version];
    
    NSString *dataVerStr = version;
    
    if ([DBXValidJudge isValidString:currentVerStr] && [DBXValidJudge isValidString:dataVerStr]) {
