...
@implementation UIColor (DBXColor)

+ (CAGradientLayer *)gradientLayerWithGradientColor:(NSString *)gradientColor gradientOrientation:(NSString *)gradientOrientation{
    CAGradientLayer *gradientLayer  = [CAGradientLayer layer];
    NSArray *colors = [gradientColor  componentsSeparatedByString:@"-"];
    NSMutableArray *colorsArray = [NSMutableArray array];
    for (NSString *color in  colors) {
        [colorsArray db_addObject:(__bridge id)[UIColor db_colorWithHexString:color].CGColor];
    }
    gradientLayer.colors = colorsArray;
    
    if ([gradientOrientation isEqualToString:@"horizontal"]) {
...
{
    if (![DBXValidJudge isValidString:hexString]) {
        return nil;
    }
