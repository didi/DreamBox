...
@interface DBXWrapperManager ()
@property (nonatomic, strong) DBXDefaultWrapper *defaultWrapper;
...
@implementation DBXWrapperManager

+ (instancetype)sharedManager {
    static DBXWrapperManager *manager = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        manager =  [[DBXWrapperManager alloc] init];
    });
    return manager;
}
#pragma mark - init
- (instancetype)init {
    self = [super init];
    if (self) {
        _defaultWrapper = [[DBXDefaultWrapper alloc] init];
...
- (void)registerAccessKey:(NSString *)accessKey wrapper:(DBXWrapper *)wrapper {
    ...
    if (wrapper && [wrapper isKindOfClass:DBXWrapper.class]) {
        wrapper.accessKey = accessKey;
        [self p_defaultsettingWrapper:wrapper];
        [self.wrapperPool db_setValue:wrapper forKey:accessKey];
    }
}
- (DBXWrapper *)getWrapperByAccessKey:(NSString *)accessKey {
    return [self p_getWrapperByAccessKey:accessKey];
}
#pragma mark -
- (DBXWrapper *)p_getWrapperByAccessKey:(NSString *)accessKey {
    if ([DBXValidJudge isValidString:accessKey] && [self.wrapperPool db_hasKey:accessKey]) {
...
- (void)getDataByTemplateId:(NSString *)tid accessKey:(NSString *)accessKey completionBlock:(DBWrapperCompletionBlock)completionBlock {
    DBXWrapper *wrapper = [self p_getWrapperByAccessKey:accessKey];
...
- (void)imageLoadService:(UIImageView *)imgView accessKey:(nonnull NSString *)accessKey setImageUrl:(NSString *)urlStr  callback:(void(^)(UIImage *image))block{
    DBXWrapper *wrapper = [self p_getWrapperByAccessKey:accessKey];
    if (wrapper && [wrapper respondsToSelector:@selector(imageLoadService:setImageUrl:callback:)]) {
        [wrapper imageLoadService:imgView setImageUrl:urlStr callback:block];
    } else {
        [self.defaultWrapper imageLoadService:imgView setImageUrl:urlStr callback:block];
    }
}
...
- (void)reportTid:(NSString *)tid Key:(NSString *)key accessKey:(nonnull NSString *)accessKey params:(NSDictionary<NSString * ,NSString *>*)params frequency:(DBReportFrequency)frequency {
    ...
    [tempDict db_setValue:[DBXDefines db_report_version] forKey:@"version"];
...
#pragma mark -
- (BOOL)p_canReport:(DBXWrapper *)wrapper {
    if (!wrapper.config) {
        return YES;// 默认YES
    } else {
...
- (BOOL)p_canFrequencyReport:(DBXWrapper *)wrapper {
    if (!wrapper.config) {
        return YES;// 默认YES
    } else {
...
- (void)p_defaultsettingWrapper:(DBXWrapper *)wrapper {
    wrapper.uuid = [DBXDefines uuidString];
    wrapper.version = [DBXDefines db_version];
}
