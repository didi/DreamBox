...
@interface DBActivityLoading : UIImageView
...
@interface DBActivityLoading ()
@property (nonatomic,assign) BOOL activityAanimating;
@end

@implementation DBActivityLoading
...
@interface DBDotLoadingView : UIView
/*!
...
@implementation DBDotLoadingView
...
- (void)setText:(NSString *)text {
    _text = text;
    BOOL hasText = [DBValidJudge isValidString:text];
...
- (NSArray *)loadingImages {
    if (!_loadingImages) {
        ...
        [images addObject:[self getImageWithBoudleName:@"DreamBox_iOS" imgName:@"common_btn_animation_normal"]];
        [images addObject:[self getImageWithBoudleName:@"DreamBox_iOS" imgName:@"common_btn_animation_left"]];
        [images addObject:[self getImageWithBoudleName:@"DreamBox_iOS" imgName:@"common_btn_animation_middle"]];
        [images addObject:[self getImageWithBoudleName:@"DreamBox_iOS" imgName:@"common_btn_animation_right"]];
...
@implementation DBLoading

-(void)setDataWithModel:(DBLoadingModel *)loadingModel andPathId:(NSString *)pathId{
    if ([loadingModel.style isEqualToString:@"dot"]) {
        DBDotLoadingView *loadingView = [DBDotLoadingView new];
        [self addSubview:loadingView];
        [loadingView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.left.width.height.mas_equalTo(self);
        }];
    }else{
        DBActivityLoading *loadingView = [DBActivityLoading new];
        [self addSubview:loadingView];
        [loadingView mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.left.width.height.mas_equalTo(self);
        }];
