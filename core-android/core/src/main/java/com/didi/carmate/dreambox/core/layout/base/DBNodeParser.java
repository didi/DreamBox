...
import com.didi.carmate.dreambox.core.layout.render.DBFrameLayout;
import com.didi.carmate.dreambox.core.layout.render.DBLinearLayout;
import com.didi.carmate.dreambox.core.layout.render.DBYogaLayout;
...
//            Wrapper.get(accessKey).monitor().crash(e);
        ...
        for (Map.Entry<String, JsonElement> entry : entries) {
            ...
            if (jsonElement.isJsonPrimitive()) {
                ...
                if (DBConstants.CONTAINER_RENDER.equals(tagName)) {
                    ...
                    collectNodeInfo(layoutNode, tagName); // 统计信息
                    looperCreateNode(subJsonObject, layoutNode);
                } else if (null != nodeCreator) {
                        ...
                        if (arrayNode instanceof DBChildren) { // 视图数组节点
                            ...
                            if (mNodeRegistry.getNodeMap().containsKey(viewTagName)) {
                                if (DBConstants.CONTAINER_CELL.equals(viewTagName) ||
                                    ...
                                    collectNodeInfo(layoutNode, viewTagName); // 统计信息
                                    looperCreateNode(subJsonObject, layoutNode); // 这里的父节点是view 节点
                                } else {
    ...
    private DBContainer<ViewGroup> getContainerNode(String layout) {
        if (DBConstants.LAYOUT_TYPE_YOGA.equals(layout)) {
            return new DBYogaLayout(mDBContext);
        } else if (DBConstants.LAYOUT_TYPE_FRAME.equals(layout)) {
            return new DBFrameLayout(mDBContext);
        } else if (DBConstants.LAYOUT_TYPE_LINEAR.equals(layout)) {
            return new DBLinearLayout(mDBContext);
        } else {
            DBLogger.e(mDBContext, "unSupport layout: " + layout);
            return null;
        }
    }
